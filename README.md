...
pipeline una vez al dia con `on.schedule` y tambien permite correrlo manualmente
desde GitHub Actions con `workflow_dispatch`.
...
Cuando se ejecuta con `--upload-s3`, el pipeline guarda particiones diarias:
...
```bash
AWS_S3_BUCKET=tu-bucket AWS_S3_PREFIX=tfm-vino-cr python -m src.pipeline --from-existing --upload-s3
```

En PowerShell:

```powershell
$env:AWS_S3_BUCKET="tu-bucket"
$env:AWS_S3_PREFIX="tfm-vino-cr"
python -m src.pipeline --from-existing --upload-s3
```
